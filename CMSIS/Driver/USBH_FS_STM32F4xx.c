...
#define USBH0_MAX_PIPE_NUM             (8U)
#endif
#if    (USBH0_MAX_PIPE_NUM > 8U)
...
static const uint32_t *OTG_DFIFO[] = { OTG_FS_DFIFO0,
                                       OTG_FS_DFIFO1,
                                       OTG_FS_DFIFO2,
                                       OTG_FS_DFIFO3,
                                       OTG_FS_DFIFO4,
...
static volatile PIPE_t pipe[USBH0_MAX_PIPE_NUM];
...
static int32_t USBH_PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
...
#else
      if (hhcd_USB_OTG_FS.Instance != NULL) {
    ...
    case ARM_POWER_FULL:
      ...
      OTG->HAINTMSK  = (1U << USBH0_MAX_PIPE_NUM) - 1U; // Enable channel interrupts
